...
def define_fitness_calculator(protocols):
    ...
    for protocol_name, locations in feature_definitions.items():
        for location, features in locations.items():
            for efel_feature_Aux in features:
                efel_feature_name, meanstd = efel_feature_Aux['feature'], efel_feature_Aux['val']
                feature_name = '%s.%s.%s' % (
...
def create(do_replace_axon=True, sim='nrn'):
    ...
    FoxP2_cell = FoxP2_model.create(do_replace_axon=do_replace_axon)
    ...
    param_names = [param.name
                   for param in FoxP2_cell.params.values()
                   if not param.frozen]
